...
namespace Airlines.XAirlines.Helpers
{
    public class CardHelper
    {
                ...
                Body = new List<AdaptiveElement>()
                 {
                    new AdaptiveContainer()
                    {
                        Items=new List<AdaptiveElement>()
                        {
                             new AdaptiveTextBlock()
                            {
                                Text=$"Hey TTT{userName}TTT! Here is what I can do for you",
        ...
        {
            var url = ApplicationSettings.BaseUrl;
            var Card = new AdaptiveCard(new AdaptiveSchemaVersion("1.0"))
            {
                Body = new List<AdaptiveElement>()
                {
        ...
        public static async Task<Attachment> GetMyDetailedCard(string code, string userEmailId, IConfiguration configuration)
        {
            ...
            if (dayPlan == null)
                return null;
            WeatherInfo weatherinfo = GetWeatherInfo(dayPlan.flightDetails.destinationCode);
